...
} from '@clients/crossProgramInvocation';
import { COUNTER_PROGRAM_ADDRESS, fetchCounter } from '@/clients/counter';
...
test.skipIf(isInitialized)(
  'basics:cross-program-invocation:create',
  async () => {
...
test('basics:cross-program-invocation:incease', async () => {
  const { defaultPayer, rpc, sendAndConfirmTransaction } = await getApi();
...
test('basics:cross-program-invocation:decrease', async () => {
  const { defaultPayer, rpc, sendAndConfirmTransaction } = await getApi();
